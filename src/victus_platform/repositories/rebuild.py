...
from domain.projections.registry import events_for_projector
    ...
    *,
    ...
    while True:
        ...
        for event in events:
            ...
            if event.event_type in events_for_projector(USER_PROFILE_PROJECTOR):
...
def rebuild_nutrition_status(
    *,
    ...
    while True:
        ...
        for event in events:
            after_seq = event.event_seq
            if event.event_type in events_for_projector(NUTRITION_STATUS_PROJECTOR):
...
def rebuild_constraint(
    *,
    ...
    while True:
        ...
        for event in events:
            after_seq = event.event_seq
            if event.event_type in events_for_projector(CONSTRAINT_PROJECTOR):
...
def rebuild_planning_history(
    *,
    ...
    while True:
        ...
        for event in events:
            after_seq = event.event_seq
            if event.event_type in events_for_projector(PLANNING_HISTORY_PROJECTOR):
