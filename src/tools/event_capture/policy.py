...
def validate_decision(
    ...
    if decision.capture_action in {"edit_meal", "delete_meal"}:
        if not decision.referenced_record and not decision.clarification_question:
            errors.append(f"{decision.capture_action} requires a record reference")
    if decision.capture_action == "log_biometrics":
        data = _decision_payload(decision, "biometrics")
        has_value = (
            bool(data.get("raw_value_text"))
            or data.get("value") is not None
            or data.get("systolic") is not None
        )
        if not data.get("metric") or not has_value:
            errors.append("log_biometrics requires a metric and value")
    if decision.capture_action == "log_lifestyle_metric":
        if not _decision_payload(decision, "lifestyle_metric").get("metric"):
            errors.append("log_lifestyle_metric requires a metric")
    if decision.capture_action == "log_symptom":
        data = _decision_payload(decision, "symptom")
        if not data.get("raw_text") and not data.get("category"):
            errors.append("log_symptom requires symptom data")
    ...
    if contains_nutrition_value_key(decision.extracted):
        errors.append("extracted payload must not contain nutrition estimates")
    if contains_high_risk_symptom(input.normalized_text) and not decision.requires_safety_validation:
...
def _decision_payload(decision: EventCaptureDecision, key: str) -> dict[str, object]:
    value = decision.extracted.get(key, {})
    return value if isinstance(value, dict) else {}


def _valid_meal(decision: EventCaptureDecision) -> bool:
    items = _decision_payload(decision, "meal").get("items", [])
    return isinstance(items, list) and bool(items) and all(
        isinstance(item, dict) and item.get("name") for item in items
    )
